...
const filters = (collection, type) => {
  console.log(collection.name);
  let obj = {
    "On sale": {
      // isMintOnSale: true,
      collectionName:
        collection.name == "Erbil Collection" ? undefined : collection?.name,
    },
    Owned: {
      collectionName: collection?.name,
      createdBy: { $ne: collection?.owner },
    },
  };
  return obj[type];
};
...
const OnSale = (props) => {
  ...
  const getAllCollectiables = ({ page, collection, type, list }) => {
    let data = {
      page,
      limit: 12,
      filter: filters(collection, type),
      isProfile: true,
    };
    setisLoading(true);
